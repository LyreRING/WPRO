...
Fig.~\ref{fig:overall} compares normalized utility, admission ratio, and on-time completion. \WPRO\ improves weighted utility by completing more valuable workflows before deadlines rather than simply admitting the largest number of requests. Fig.~\ref{fig:scale} shows that the advantage grows under heavy load, where cold-load mistakes and future ready-stage releases become more consequential.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../paper_artifacts/figures/fig1_overall_performance.pdf}
\caption{Overall performance. \WPRO\ improves weighted utility and on-time completion while maintaining disciplined admission.}
\label{fig:overall}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../paper_artifacts/figures/fig2_scalability_vs_arrival_rate.pdf}
\caption{Scalability with arrival rate \(\lambda\). Future-aware orchestration becomes more valuable under heavy load.}
\label{fig:scale}
\end{figure}

Fig.~\ref{fig:mechanism} validates the mechanism. The demand head estimates DAG-induced near-future demand, and higher demand awareness translates into improved residency hit ratio. Fig.~\ref{fig:ablation} shows that progress, demand, residency, WAIT, and shaping each contribute to performance; demand and residency are especially important under heavy load.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../paper_artifacts/figures/fig6_demand_prediction_and_residency.pdf}
\caption{Mechanism analysis: demand estimation and residency hit ratio.}
\label{fig:mechanism}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../paper_artifacts/figures/fig7_ablation_study.pdf}
\caption{Ablation study. Removing future demand or residency features substantially weakens \WPRO.}
\label{fig:ablation}
\end{figure}

Fig.~\ref{fig:overhead} illustrates scheduling behavior and runtime overhead. \WPRO\ reduces unnecessary cold loads while keeping event-level decision overhead low because the autoregressive decoder avoids full dispatch-matrix enumeration.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../paper_artifacts/figures/fig8_schedule_timeline_and_overhead.pdf}
